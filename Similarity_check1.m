function Rs = Similarity_check1(Tasks,pop,gen,selection_process,rmp,p_il,reps,transfer1,transfer2)
%MFEA function: implementation of MFEA algorithm
    ...
    for rep = 1:reps
        ...
        for i = 1 : pop
            ...
            x(x<0)=0;
            x(x>1)=1;
            population1(i).rnvec = x;
            population2(i) = population(i);
            x = population(i).rnvec + transfer2;
            x(x<0)=0;
            x(x>1)=1;
            population2(i).rnvec = x;
        end
        for i = 1 : pop
...
%                         s1(i) = (population(i).factorial_ranks(1)-population(i).factorial_ranks(2));
            X(i) = population1(i).factorial_costs(1);
            Y(i) = population2(i).factorial_costs(2);
        end
%         s1 = 6*sum(s1.^2);
%         Rs = 1-s1/(pop^3-pop);
